...
function toZecatLike(p = {}) {
  const imgs = (p.images || []).map(u =>
    typeof u === 'string'
      ...
      : { image_url: u.image_url || u.url, main: !!u.main, main_integrator: !!u.main_integrator }
  )
  return {
...
export async function getServerSideProps({ params }) {
  ...
  try {
    ...
    if (fam) {
      const url = `${API_BASE}/api/store/products?limit=12&family=${encodeURIComponent(fam)}`
      console.log('[GSSP] relacionados →', url)
      try {
        const r = await fetch(url)
        if (r.ok) {
          const rel = await r.json()
          const list = Array.isArray(rel.items) ? rel.items : (rel.products || [])
          related = list
  ...
  return (
    <>
      ...
      <main className="max-w-7xl mx-auto px-4 py-12 space-y-12">
        ...
        {related?.length > 0 && (
          <RelatedProductsCarousel productos={related} />
        )}
      </main>
      <Footer />
    </>
